...
# Domains known to host preprints — work that has NOT been peer reviewed,
# regardless of how polished or well-cited it looks.
PREPRINT_DOMAINS = {"arxiv.org", "biorxiv.org", "medrxiv.org", "ssrn.com"}

PREPRINT_PENALTY = -0.15
...
def _fetch_page(url: str, timeout: float = 5.0) -> Optional[BeautifulSoup]:
    """
    Fetch a URL and parse it as HTML. Returns None on any failure — a timeout,
    a dead link, a non-200 status, or a page that isn't HTML — so a single bad
    URL cannot crash scoring. Mirrors the fail-soft pattern used by llm_opinion().
    """
    try:
        response = requests.get(
            url,
            timeout=timeout,
            headers={"User-Agent": "Mozilla/5.0 (credibility-scorer)"},
        )
        ...
        return None
def content_signals(url: str) -> List[Signal]:
    """
    Fetch the actual page and look for editorial signals the URL string alone
    cannot show: a named author, a publish date, and a references/citations
    section. Returns an empty list if the page cannot be fetched, so callers
    don't need special-case handling for a dead link.
    """
    ...
    # Signal: a named author, via a <meta name="author"> tag or a common
    # byline pattern in the visible text.
    has_meta_author = soup.find("meta", attrs={"name": "author"}) is not None
    has_byline_text = bool(re.search(r"\bby\s+[A-Z][a-z]+\s+[A-Z][a-z]+", soup.get_text(" ", strip=True)))
    if has_meta_author or has_byline_text:
        signals.append(Signal("has_author", 0.05, "page shows a named author"))

    # Signal: a publish or updated date, via common meta tags or a <time> tag.
    has_date_meta = soup.find("meta", attrs={"property": "article:published_time"}) is not None
    has_time_tag = soup.find("time") is not None
    if has_date_meta or has_time_tag:
        signals.append(Signal("has_date", 0.05, "page shows a publish date"))

    # Signal: mentions of references/citations/sources, suggesting the claims
    # are traceable rather than asserted with no backing.
    if any(word in text for word in ("references", "citations", "bibliography")):
        signals.append(Signal("has_references", 0.05, "page includes references or citations"))
...
def rule_based_signals(url: str) -> List[Signal]:
    ...
    if re.search(r"/10\.\d{4,9}/", path):
        ...
        # Signal 6: preprint servers host unreviewed work. This penalty applies
    # regardless of other positive signals (references, DOI, etc.), since
    # having citations does not mean the work has been peer reviewed.
    if domain in PREPRINT_DOMAINS or any(domain.endswith("." + d) for d in PREPRINT_DOMAINS):
        signals.append(Signal("preprint", PREPRINT_PENALTY, "hosted on a preprint server, not peer reviewed"))
...
def score_url(url: str, use_llm: Optional[bool] = None) -> Dict[str, Any]:
    ...
    # Layer 1 always runs: URL-string rules plus, when the page is reachable,
    # signals pulled from the actual page content (author, date, references).
    signals = rule_based_signals(url) + content_signals(url)
